...
<div class="flex mb-4">
    @if ($label)
        <div
            class="flex {{ $labelTextAlign === 'left' ? 'justify-start' : 'justify-end' }} items-center px-2 bg-gray-200 border border-r-2 border-gray-300 rounded-l-md dark:bg-gray-600 dark:border-gray-600 w-1/4">
    ...
    <input type="text" id="{{ $idName }}" name="{{ $idName }}" {{ $disabled == 1 ? 'disabled' : '' }}
        ...
        dark:focus:ring-blue-500 dark:focus:border-blue-500 focus:outline-none"
    ...
    @if ($icon)
        <button wire:click="changeIcon"
            ...
            <livewire:components.icon-component :iconName="$icon[$currentIconIndex]" />
        </button>
    @endif
    @error($idName)
